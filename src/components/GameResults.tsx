...
const GameResults = ({ gameData, onRestart, userMode }: GameResultsProps) => {
  ...
  }, [gameData, saveGameSession, updateUserProgress]);
  ...
  const getEncouragementMessage = () => {
    if (gameData.accuracy >= 90) {
      return 'Outstanding security detective work! Your instincts are exceptional.';
    } else if (gameData.accuracy >= 80) {
      return 'Excellent work! You\'re becoming a true cybersecurity expert.';
    } else if (gameData.accuracy >= 70) {
      return 'Great job! Your security awareness is growing stronger.';
    } else if (gameData.accuracy >= 60) {
      return 'Good progress! Every scenario teaches valuable lessons.';
    } else {
      return 'Every expert started as a beginner. Keep learning and growing!';
    }
  };

  const getImprovementTip = () => {
    if (gameData.accuracy < 70) {
      return 'Focus on understanding the context clues in each scenario.';
    } else if (gameData.accuracy < 85) {
      return 'Pay attention to timing and location patterns in the logs.';
    } else {
      return 'You\'re mastering the art of threat detection!';
    }
  };
